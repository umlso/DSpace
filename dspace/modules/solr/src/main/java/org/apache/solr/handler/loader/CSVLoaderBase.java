...
abstract class CSVLoaderBase extends ContentStreamLoader {
    ...
    private class FieldAdderEmpty extends FieldAdder {
        @Override
        void add(SolrInputDocument doc, int line, int column, String val) {
            doc.addField(fieldnames[column],val,1.0f);
        }
    ...
    /** trim fields */
    private class FieldTrimmer extends FieldAdder {
        private final FieldAdder base;
        FieldTrimmer(FieldAdder base) { this.base=base; }
        @Override
        void add(SolrInputDocument doc, int line, int column, String val) {
     ...
     * using a HashMap.
     */
    private class FieldMapperSingle extends FieldAdder {
     ...
     * a CSVStrategy.
     */
    private class FieldSplitter extends FieldAdder {
    ...
    void prepareFields() {
        ...
        adders = new FieldAdder[fieldnames.length];
        ...
        FieldAdder adderKeepEmpty = new FieldAdderEmpty();
        ...
        for (int i=0; i<fieldnames.length; i++) {
            ...
            if (fmap!=null) {
                for (String mapRule : fmap) {
                    ...
                    adders[i] = new FieldMapperSingle(mapArgs[0], mapArgs[1], adders[i]);
                }
            }

            if (params.getFieldBool(fname,TRIM,false)) {
                adders[i] = new FieldTrimmer(adders[i]);
            }
            ...
            if (params.getFieldBool(fname,SPLIT,false)) {
                ...
                //atmire change. info:https://atmire.com/internalforum/viewtopic.php?f=6&t=114&sid=ab5d6efaef2897f3936989d30ed94c08#p247
                String encStr = null;
                ...
                adders[i] = new FieldSplitter(fstrat, adders[i]);
            }
        }
