...
import com.atmire.dspace.app.plugins.ContextPlugin;
import com.atmire.dspace.app.xmlui.aspect.administrative.FlowItemUtilsPlugin;
...
public class FlowItemUtils
{
    ...
    private static List<FlowItemUtilsPlugin> pluginSequence = new DSpace().getServiceManager().getServicesByType(FlowItemUtilsPlugin.class);
    ...
    public static FlowResult processAddBitstream(Context context, int itemID, Request request) throws SQLException, AuthorizeException, IOException
    {
        ...
        if (filePart != null && filePart.getSize() > 0)
        {
            ...
            for (FlowItemUtilsPlugin plugin : pluginSequence) {
                if(plugin instanceof ContextPlugin){
                    ((ContextPlugin) plugin).setContext(context);
                }
                (plugin).addExtraMetadata(bitstream);
            }
     ...
     */
    public static FlowResult processEditBitstream(Context context, int itemID, int bitstreamID, String bitstreamName, String primary, String description, int formatID, String userFormat, RequestWrapper request) throws SQLException, AuthorizeException
    {
        FlowResult result = new FlowResult();
        result.setContinue(false);
        ...
        for (FlowItemUtilsPlugin plugin : pluginSequence) {
            ...
            (plugin).processEnabledFlowItemUtils(request, bitstream);
        }
    ...
    public static FlowResult processReorderBitstream(Context context, int itemID, Request request) throws SQLException, AuthorizeException {
        ...
        for (Bundle bundle : bundles) {
            ...
            if (submitButton.equals("submit_update_order")) {
                ...
                String[] buttonSplit = submitButton.split("_");
                int buttonStreamID = Integer.parseInt(buttonSplit[3]);
                String direction = buttonSplit[4];
                int relPos = 0;
                ...
                for(Bitstream bs : bitstreams) {
                    tempList.add(bs.getID());

                    if(bs.getID() == buttonStreamID) {
                        pos = tempList.size() - 1;
                    }
                }

                int otherPos = pos + relPos;

                if(otherPos >= tempList.size()) {
                    otherPos = 0;
                }
                else if(otherPos < 0) {
                    otherPos = tempList.size() - 1;
                }

                int insert = otherPos < pos ? tempList.remove(otherPos) : tempList.remove(pos);
                tempList.add(otherPos > pos ? otherPos : pos, insert);

                for(int counter = 0; counter < tempList.size() && counter < newBitstreamOrder.length; counter++) {
                    newBitstreamOrder[counter] = tempList.get(counter);
                }
            }
